...
//總收入
const findRate =
  "select customerid ,MONTH(max(time)) as t,MONTH(min(time)) as h from transaction  group by customerid order by t asc;";
...
const findCustomer =
  "SET @t = 7;SELECT count(distinct customerId) as amount ,month(time) as month , IF(month(time)=7, @t:= month(time) , @t := month(time)-1) as q FROM transaction where customerId IN (select distinct customerId from transaction where month(time)=@t) group by month(time) ;";
const findIndex =
  "SELECT count(distinct customerId) as cAmount ,month(time) as month FROM transaction group by month(time);";
  ...
  await db.query(findIndex, (err, result) => {
    ...
    db.query(findCustomer, (err, result) => {
      ...
      for (let i = 0; i < result[1].length; i++) {
        ...
        totalArray.push({
          month: result[1][i].month,
          retetionRate: rArray[i] * 100 + "%",
          defectionRate: defectionArray[i] * 100 + "%",
          surviveRate: Math.floor(surviveArray[i] * 100) + "%",
        });
      }
      res.send(totalArray);
    });
