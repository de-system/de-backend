...
const checkROPandEOQ = async () => {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (stock <= ROP && DateDiff(lastOrderDate, currentDate) > 7) {
            orderId += 1;
            receiveDate = new Date(data[i].time).addDays(7).toISOString().split('T')[0];
            lastOrderDate = data[i].time.toISOString().split('T')[0];
            list.push({ orderId: orderId, orderDate: lastOrderDate, receiveDate: receiveDate, EOQf: calculateEOQ()[0], EOQa: calculateEOQ()[1] });
        }
    }
    JSON.stringify(list);
    return list;
...
const showStock = "SELECT pName , sum(stock) as totalStock FROM product INNER JOIN hasStock USING (productId)";
